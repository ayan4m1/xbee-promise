...
module.exports = function xbeePromiseLibrary(options) {
    ...
    function closeSerialport() {
        ...
        port.drain(function closeAfterDraining() {
            port.close();
        });
    }
    ...
    function _sendFramePromiseResponse(frame, timeoutMs, responseFrameType) {
        ...
        // Write to the serial port (when open or now if open)
        if (port.paused) {
            port.on("open", function () {
                port.write(xbeeApi.buildFrame(frame));
            });
        } else {
            port.write(xbeeApi.buildFrame(frame));
        }
    ...
    function _localCommand(command, timeoutMs, commandParameter) {
        var frame = {
                type: api.constants.FRAME_TYPE.AT_COMMAND,
                command: command,
        ...
        return _sendFramePromiseResponse(frame, timeoutMs, api.constants.FRAME_TYPE.AT_COMMAND_RESPONSE)
            .then(function (frame) {
                if (frame.commandStatus === api.constants.COMMAND_STATUS.OK) {
                    return frame.commandData;
                }

                // if not OK, throw error
                throw new Error(api.constants.COMMAND_STATUS[frame.commandStatus]);
            });
    }
    ...
    function _remoteCommand(command, destination64, destination16, timeoutMs, commandParameter) {
        var frame = {
                type: api.constants.FRAME_TYPE.REMOTE_AT_COMMAND_REQUEST,
                command: command,
                commandParameter: commandParameter,
        ...
        return _sendFramePromiseResponse(frame, timeoutMs, api.constants.FRAME_TYPE.REMOTE_COMMAND_RESPONSE)
            .then(function (frame) {
                if (frame.commandStatus === api.constants.COMMAND_STATUS.OK) {
                    return frame.commandData;
                }
    ...
    function _remoteTransmit(destination64, destination16, data, timeoutMs) {
        var frame = {
                data: data,
                type: api.constants.FRAME_TYPE.ZIGBEE_TRANSMIT_REQUEST,
                destination64: destination64,
                destination16: destination16
            },
            responseFrameType = api.constants.FRAME_TYPE.ZIGBEE_TRANSMIT_STATUS;

        if (module === "802.15.4") {
            responseFrameType = api.constants.FRAME_TYPE.TX_STATUS;
            frame.type = destination64 ?
                    api.constants.FRAME_TYPE.TX_REQUEST_64 :
                    api.constants.FRAME_TYPE.TX_REQUEST_16;
        }
        ...
        return _sendFramePromiseResponse(frame, timeoutMs, responseFrameType)
            .then(function (frame) {
                if (frame.deliveryStatus === api.constants.DELIVERY_STATUS.SUCCESS) {
                    return true;
                }

                // if not OK, throw error
                throw new Error(api.constants.DELIVERY_STATUS[frame.deliveryStatus]);
            });
    }
    ...
    xbeeApi = new api.XBeeAPI({
        apiMode: options.apiMode || 1,
        module: module
    });

    options.portOptions = options.portOptions || {};

    options.portOptions.parser = xbeeApi.rawParser();
    ...
    if (debug) {
        console.log("Connecting to serialport", options.portName);
    }
    port = new SerialPort(options.portName, options.portOptions);
